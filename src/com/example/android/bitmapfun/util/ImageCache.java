...
public class ImageCache {
    ...
    private static Map<String, ImageCache> sImageCacheMap = new HashMap<String, ImageCache>();
    ...
    public static ImageCache getInstance( String id, ImageCacheParams cacheParams) {
        ImageCache imageCache = sImageCacheMap.get(id);

        // No existing ImageCache, create one and store it in RetainFragment
        if (imageCache == null) {
            imageCache = new ImageCache(cacheParams);
            sImageCacheMap.put(id, imageCache);
        }
     ...
     */
    public void addBitmapToCache(BitmapData data, BitmapDrawable value) {
        ...
        if (mMemoryCache != null) {
            ...
            mMemoryCache.put(data.getMemKey(), value);
        }

        synchronized (mDiskCacheLock) {
            // Add to disk cache
            if (mDiskLruCache != null) {
                final String key = hashKeyForDisk(data.getDiskKey());
     ...
     */
    public BitmapDrawable getBitmapFromMemCache(BitmapData data) {
        BitmapDrawable memValue = null;

        if (mMemoryCache != null) {
            memValue = mMemoryCache.get(data.getMemKey());
        }
     ...
     */
    public Bitmap getBitmapFromDiskCache(BitmapData data) {
        final String key = hashKeyForDisk(data.getDiskKey());
        Bitmap bitmap = null;
                    ...
                    if (snapshot != null) {
                        ...
                        if (inputStream != null) {
                            ...
                            bitmap = BitmapUtils.decodeSampledBitmapFromDescriptor(
                                    fd, data.getDesiredWidth(), data.getDesiredHeight(), this);
                        }
                    }
                } catch (final IOException e) {
